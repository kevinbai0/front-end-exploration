import { Dimension, ThemeExtension, ThemeObject, DNA } from '../theme/types';
...
export const matchDimension = (
  ...
  prop: string | number | (number | string)[]
) => {
...
export const injectDimensions: InjectProperties<Dimension> = (
  props,
  defaultProps
) => {
  return [
    splitStyle(
      'width',
      ['width'],
      (_, prop) => matchSizeProp(_, prop, 'width'),
      props,
      defaultProps
    ),
    splitStyle(
      'height',
      ['height'],
      (_, prop) => matchSizeProp(_, prop, 'height'),
      props,
      defaultProps
    ),
