...
export const matchSpaceToTheme = (
  props: DNA<ThemeExtension> & ThemeObject<ThemeExtension>,
  space:
    | number
    | number[]
    | ThemeSpace<ThemeExtension>
    | ThemeSpace<ThemeExtension>[]
    | string
    | string[]
) => {
  ...
  if (typeof space === 'string') {
    const themeSpace = (props.theme.space[
  ...
  const expansion = (space as ThemeSpace<ThemeExtension>[]).map((value) => {
    ...
    if (typeof themeSpace === 'number')
      return props.theme.breakpoints.map(() => themeSpace);
...
export const injectSpace: InjectProperties<Spacing<ThemeExtension>> = (
  props,
  defaultProps
) => {
  return [
